...
/*
 * Collapsed SAEGW-C S8 relay (home-routed roaming) scenario.
 *
 * The test UE's IMSI (999708...) matches a forced MME PGW-selection rule
 * pointing at 127.0.1.4 - the SMF's second GTP-C address, which is NOT
 * its advertised address. The SMF therefore classifies the S11 Create
 * Session Request as home-routed and plays the SGW-C role: it programs
 * the UPF as a pure GTP-U forwarder and relays the Create Session Request
 * over S5/S8 to 127.0.1.4 - where the same SMF anchors the session in its
 * normal PGW role (loopback home PGW).
 *
 * The flow is the standard attach + user-plane ping + detach, but the
 * user plane traverses the UPF twice:
 *   eNB -> UPF (relay UL) -> UPF (anchor UL) -> ogstun -> ...
 *   ... -> UPF (anchor DL) -> UPF (relay DL) -> eNB
 */
...
static void test1_func(abts_case *tc, void *data)
{
    ...
    /* IMSI 999707864001000 (MSIN digits are BCD-swapped): prefix
     * 99970786 matches the forced "home PGW" PGW-selection rule in
     * collapsed.yaml (S8 relay). */
    test_ue = test_ue_add_by_suci(&mobile_identity_suci, "8746000100");
    ogs_assert(test_ue);
     ...
     * (default bearer established via the S8 relay + loopback home PGW) */
    recvbuf = testenb_s1ap_read(s1ap);
    ABTS_PTR_NOTNULL(tc, recvbuf);
    tests1ap_recv(test_ue, recvbuf);
    ...
    /* Send GTP-U Router Solicitation (learns the IPv6 prefix via RA) */
    ...
    /* Send GTP-U ICMP Packet (traverses relay + anchor UPF sessions) */
    ...
    /* Send Detach Request (relayed as S5 Delete Session to the home PGW) */
...
abts_suite *test_relay(abts_suite *suite)
{
