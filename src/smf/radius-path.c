 ...
 *
 * This used to call OpenSSL EVP_Digest(). That crashed the SMF at
 * process exit: libcrypto registers its atexit() cleanup on first lazy
 * use, which happens AFTER main() registers atexit(terminate), so on
 * shutdown OPENSSL_cleanup runs BEFORE smf_context_final(). The final
 * RADIUS Accounting-Stop sent for every active session then called
 * EVP_Digest() on destroyed provider state -> SIGSEGV -> multi-GB core
 * dump -> systemd 'stop-sigterm' timeout -> SIGKILL, turning every SMF
 * restart into minutes. A local MD5 has no library state to tear down.
 */
...
static void md5_digest(const void *data, size_t len, uint8_t out[16])
{
    ...
    while (len - (size_t)(p - (const uint8_t *)data) >= 64) {
        md5_block(st, p);
        p += 64;
    }

    rem = len - (size_t)(p - (const uint8_t *)data);
    memset(tail, 0, sizeof(tail));
    memcpy(tail, p, rem);
    tail[rem] = 0x80;
    tail_len = (rem < 56) ? 64 : 128;
    for (i = 0; i < 8; i++)
        tail[tail_len - 8 + i] = (uint8_t)(nbits >> (8 * i));
    md5_block(st, tail);
    if (tail_len == 128)
        md5_block(st, tail + 64);

    for (i = 0; i < 16; i++)
        out[i] = (uint8_t)(st[i / 4] >> (8 * (i & 3)));
}
