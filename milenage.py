...
#
# -----------------------------------------------------------------------------
# K4 patch (Huawei HSS9860 stored-credential unwrap)
#
# Mirrors the Open5GS lib/crypt/milenage.c K4 patch: K and OPc in the DB are
# AES-128-ECB encrypted with a K4 key. At each Milenage entry point the stored
...
def _k4_init():
    ...
    for var in ("PYHSS_K4", "OPEN5GS_K4"):
        hexv = os.environ.get(var)
        if not hexv:
            continue
        if len(hexv) != 32:
            CryptoLogger.warning(
                "Milenage K4: %s is set but not 32 hex chars, ignoring", var)
            continue
        try:
            _k4_key = bytes.fromhex(hexv)
            _k4_source = var
            break
        except ValueError:
            CryptoLogger.warning(
    ...
    CryptoLogger.info(
        "Milenage K4: source=%s enabled=%s", _k4_source, _k4_enabled)


def _k4_unwrap(k, opc):
    """
    Unwrap (K_stored, OPc_stored) -> (K_plain, OPc_plain).
    Pass-through when K4 is all zeros. Input may be bytes or bytearray.
    """
    _k4_init()
    if not _k4_enabled:
        return k, opc
    cipher = AES.new(_k4_key, AES.MODE_ECB)
    return cipher.decrypt(bytes(k)), cipher.decrypt(bytes(opc))
