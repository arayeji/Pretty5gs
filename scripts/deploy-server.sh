#!/usr/bin/env bash
# Full rebuild + install Open5GS on THR1EPC01 (or any host with systemd units).
# Run on the server as a user with passwordless sudo, from any directory:
#   bash /home/superadmin/open5gsNew/scripts/deploy-server.sh
#
# Prerequisite: push fixes to github/main first, then run this script on the server.

set -euo pipefail

REPO_DIR="${OPEN5GS_REPO:-/home/superadmin/open5gsNew}"
